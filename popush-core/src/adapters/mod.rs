//! Service adapters. Each adapter knows how to check, start, stop, restart,
//! and tail logs for one service type, and, crucially, what it *cannot* do,
//! via [`Capabilities`]. `capabilities` is what stops the UI lying: a
...
//! output**, because the parse is where honesty is won or lost.
...
/// What an adapter can actually do. Drives which buttons render.
